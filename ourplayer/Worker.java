...
public class Worker extends MyUnit {
  ...
  ArrayList<ResourceInfo> found_resources =
      new ArrayList<>(); // ArrayList instead of queue in case we want to prioritize certain
  // resources
  ...
  private void decodeResourceMessages() {
    ...
    // If a smoke signal is from an ally and is for a resource location, add the resource to the
    // found_resources.
  ...
  void senseResources() {
    ...
    // If sensed resources haven't been added yet, add them to the list, and if there is a large
    // amount of
  ...
  private boolean alreadyRecordedResource(ResourceInfo resource) {
    for (ResourceInfo info : this.found_resources) {
      if (info.resourceType == resource.resourceType
          && sameLocation(info.location, resource.location)) {
        return true;
      }
    }
    return false;
  ...
  void keepTorchLit() {
    ...
    if (torchRounds == 0) {
      if (uc.canLightTorch()) uc.lightTorch();
    }
    // After initial torch light, throw torch on ground and light a new one when the torch is almost
    // depleted.
    // TODO: Check limit
      ...
      if (dropTorch())
        if (uc.canLightTorch()) {
          uc.lightTorch();
        } else uc.println("SOMEHOW TORCH CAN NOT BE LIT");
    }
  }
  ...
  void deposit() {
    ...
    if (bug2(uc.getLocation(), depositLocation)) {
      if (uc.canDeposit()) {
        // Ensure that one barrack is placed near the unit's deposit location once barracks are
        // unlocked.
        if (uc.hasResearched(Technology.MILITARY_TRAINING, team)) {
          if (!checkForBarrack()) spawnRandom(UnitType.BARRACKS);
        }
  ...
  private boolean checkForBarrack() {
    ...
    for (UnitInfo unit : unitsNearby) {
      if (unit.getTeam() == team) {
        if (unit.getType() == UnitType.BARRACKS) return true;
      }
    }
      ...
      if (!messagesToSend.isEmpty()) {
        if (maxResourcesCarried == 0
            && ((maxResourceAmountAtLocation >= 100 && !uc.hasResearched(Technology.BOXES, team))
                || (maxResourceAmountAtLocation >= 200
                    && uc.hasResearched(Technology.BOXES, team)))) {
  ...
  /**
   * Helper function for mine(). Exists since multiple resources can be stacked on top of eachother
   * (e.g. deer dies on a forest).
   *
  ...
  private int max(ResourceInfo[] array) {
    ...
    for (ResourceInfo i : array) {
      if (i != null && i.amount > max) max = i.amount;
    }
  ...
  /**
   * Helper function for mine(). Exists since we are determining whether a worker goes to deposit if
   * it fills up on one of its resources.
   *
  ...
  void moveToResource() {
    ...
    // If the list of found locations is not empty and a location isn't set, set target location to
    // the first one
  ...
  void spawnBuildings() {
    ...
    if (uc.hasResearched(Technology.MILITARY_TRAINING, team)) {
      if (barracks < 1) {
        if (spawnRandom(UnitType.BARRACKS)) barracks++;
      }
    }
