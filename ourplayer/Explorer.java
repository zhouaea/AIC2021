...
public class Explorer extends MyUnit {
    ...
    void playRound() {
        ...
        if (this.newResources.size() > 0) {
            this.uc.println("Resource at: " + this.newResources.get(0));

            if (this.uc.canMakeSmokeSignal()) {
                int signal = encodeResourceMessage(this.newResources.remove(0));
                this.uc.makeSmokeSignal(signal);
                this.uc.println(
        ...
        if (!this.baseFound) {
            if (this.findBase()) {
                ...
                this.baseFound = true;
                this.uc.println("\n\n\n\n\n\n\nEnemy base found at " + this.enemyBaseLocation);
            }
        }
...
//  }
    ...
    // ignores low resource stores (less than 100)
    boolean findResources() {
        for (ResourceInfo info : this.uc.senseResources()) {
            Location l = info.getLocation();
